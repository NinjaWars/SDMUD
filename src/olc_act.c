...
const struct olc_help_type help_table[] = {
    ...
    {"act2", act2_flags, "Mobile attributes x2."},
...
/*    buf1[0] = '\0'; */
    ...
    page_to_char ( buffer_string ( buf1 ), ch );
    //free_buf ( buf1 );
    buffer_free ( buf1 );
    return FALSE;
}
...
REDIT ( redit_show )
{
    ...
    sprintf ( buf, "Clan : [%d] %s\n\r", pRoom->clan,
              ( ( pRoom->clan >
                  0 ) ? clan_table[pRoom->clan].name : "none" ) );
    strcat ( buf1, buf );
    ...
    for ( door = 0; door < MAX_DIR; door++ )
    {
        ...
        if ( ( pexit = pRoom->exit[door] ) )
        {
            ...
            sprintf ( buf, "-%-5s to [%ld] Key: [%ld] ", capitalize ( dir_name[door] ), pexit->u1.to_room ? pexit->u1.to_room->vnum : 0,    /* ROM OLC */
                      pexit->key );
            strcat ( buf1, buf );
...
REDIT ( redit_tele )
{
    ...
    if ( is_number ( argument ) )
    {
        ...
        send_to_char ( "Teleport Destination set.\n\r", ch );
        return TRUE;
    }

    send_to_char ( "Syntax : tele <#xnumber>\n\r", ch );
    return FALSE;
}
...
REDIT ( redit_oreset )
{
    ...
    if ( argument[0] == '\0' &&
             ( ( to_obj =
                     get_obj_list ( ch, arg2, pRoom->contents ) ) != NULL ) )
    {
        pReset = new_reset_data (  );
        pReset->command = 'P';
        pReset->arg1 = pObjIndex->vnum;
    ...
    if ( ( to_mob = get_char_room ( ch, arg2 ) ) != NULL )
    {
         ...
         */
        if ( ( wear_loc =
               flag_value ( wear_loc_flags, argument ) ) == NO_FLAG )
        {
        ...
        {
            sprintf ( output, "%s (%ld) has wear flags: [%s]\n\r",
                      capitalize ( pObjIndex->short_descr ), pObjIndex->vnum,
    ...
    switch ( obj->item_type )
    {
        ...
        case ITEM_SLOT_MACHINE:
            sprintf ( buf,
                      "[v0] Platinum Cost   [%ld]\n\r"
                      "[v1] Jackpot Value [%ld]\n\r"
                      "[v2] Number of bars [%ld]\n\r"
                      "[v3] Partial Jackpot [%ld]\n\r"
                      "[v4] Freeze Jackpot [%ld]\n\r", obj->value[0],
...
/* It somehow fixed a bug in showing scroll/pill/potions too ?! */
        ...
        case ITEM_CONTAINER:
            sprintf ( buf,
                      ...
                      get_obj_index ( obj->value[2] ) ? get_obj_index ( obj->
                                                                        value
                                                                        [2] )->
                      ...
                      char *argument )
{
    switch ( pObj->item_type )
    {
        ...
        case ITEM_SLOT_MACHINE:
            ...
            {
                case 0:
                    send_to_char ( "PLATINUM COST SET.\n\r\n\r", ch );
                ...
                case 4:
                    ...
                    pObj->value[4] ^=
                        ( flag_value ( weapon_type2, argument ) !=
                          NO_FLAG ? flag_value ( weapon_type2,
                                                 argument ) : 0 );
                    break;
            }
            break;
                ...
                case 2:
                    ...
                    pObj->value[2] ^=
                        ( flag_value ( furniture_flags, argument ) !=
                          NO_FLAG ? flag_value ( furniture_flags,
                                                 argument ) : 0 );
                    break;
                case 3:
            ...
            switch ( value_num )
            {
                ...
                case 2:
                    if ( atol ( argument ) != 0 )
                    {
                        ...
                        if ( get_obj_index ( atol ( argument ) )->item_type !=
                             ITEM_KEY )
                        {
...
/*
    ...
    for ( cnt = 0, paf = pObj->affected; paf; paf = paf->next )
    {
        if ( cnt == 0 )
        {
            send_to_char ( "Number Modifier Affects      Special Type\n\r",
                           ch );
            send_to_char ( "------ -------- -------      ------- ----\n\r",
                           ch );
        }
        sprintf ( buf, "[%4d] %-8d %-12s %-7s %s\n\r", cnt, paf->modifier,
                  flag_string ( apply_flags, paf->location ),
                  paf->where == TO_AFFECTS ? "affect" : paf->where ==
                  TO_SHIELDS ? "shield" : paf->where ==
                  TO_IMMUNE ? "immune" : paf->where ==
                  TO_RESIST ? "resist" : paf->where == TO_VULN ? "vuln" : " ",
                  paf->where ==
                  TO_AFFECTS ? affect_bit_name ( paf->bitvector ) : paf->
                  where ==
                  TO_SHIELDS ? shield_bit_name ( paf->bitvector ) : paf->
                  where ==
                  TO_IMMUNE ? imm_bit_name ( paf->bitvector ) : paf->where ==
                  TO_RESIST ? imm_bit_name ( paf->bitvector ) : paf->where ==
                  TO_VULN ? imm_bit_name ( paf->bitvector ) : " " );
        send_to_char ( buf, ch );
        cnt++;
    }
...
OEDIT ( oedit_addaffect )
{
    ...
    if ( !str_prefix ( loc, "spells" ) )
    {
        int bit = 0, pos, i;

        for ( pos = 0; affect_flags[pos].name != NULL; pos++ )
        {
            if ( !str_cmp ( affect_flags[pos].name, mod ) )
                bit = affect_flags[pos].bit;
        }

        if ( bit == 0 )
        {
            send_to_char ( "Invalid spell affect. \n\r", ch );
            send_to_char ( "Type '? affect' for a list of spell affects.\n\r",
                           ch );
    ...
    if ( !str_prefix ( loc, "shield" ) )
    {
        int bit = 0, pos, i;

        for ( pos = 0; shield_flags[pos].name != NULL; pos++ )
        {
            if ( !str_cmp ( shield_flags[pos].name, mod ) )
                bit = shield_flags[pos].bit;
        }

        if ( bit == 0 )
        {
            send_to_char ( "Invalid shield affect. \n\r", ch );
            send_to_char ( "Type '? shield' for a list of spell affects.\n\r",
                           ch );
        ...
        send_to_char ( "Shield added.\n\r", ch );
        return TRUE;
    }

    if ( !str_prefix ( loc, "immune" ) )
    {
        int bit = 0, pos;

        for ( pos = 0; imm_flags[pos].name != NULL; pos++ )
        {
            if ( !str_cmp ( imm_flags[pos].name, mod ) )
                bit = imm_flags[pos].bit;
        }

        if ( bit == 0 )
        {
            send_to_char ( "Invalid immunity. \n\r", ch );
            send_to_char ( "Type '? imm' for a list of immunities.\n\r", ch );
            return FALSE;
        }
        ...
        send_to_char ( "Immunity added.\n\r", ch );
        return TRUE;
    }

    if ( !str_prefix ( loc, "resist" ) )
    {
        int bit = 0, pos;

        for ( pos = 0; res_flags[pos].name != NULL; pos++ )
        {
            if ( !str_cmp ( res_flags[pos].name, mod ) )
                bit = res_flags[pos].bit;
        }

        if ( bit == 0 )
        {
            send_to_char ( "Invalid resistance. \n\r", ch );
            send_to_char ( "Type '? res' for a list of resistances.\n\r",
                           ch );
            return FALSE;
        }
    ...
    if ( !str_prefix ( loc, "vuln" ) )
    {
        int bit = 0, pos;

        for ( pos = 0; vuln_flags[pos].name != NULL; pos++ )
        {
            if ( !str_cmp ( vuln_flags[pos].name, mod ) )
                bit = vuln_flags[pos].bit;
        }

        if ( bit == 0 )
        {
            send_to_char ( "Invalid vulnerability. \n\r", ch );
            send_to_char ( "Type '? vul' for a list of vulnerabilities.\n\r",
                           ch );
            return FALSE;
        }
...
{
    send_to_char
        ( "Use addaffect spells/shield <spellaffect/shieldaffect>.\n\r", ch );
    send_to_char
        ( "Use addaffect [immune/resist/vuln] <imm/resist/vuln>.\n\r", ch );
    return FALSE;
...
OEDIT ( oedit_autoweapon )
{
    ...
    {
        send_to_char
            ( " {cAutoweapon requires a level to be set on the weapon first.{x\n\r",
              ch );
        return FALSE;
    }
...
OEDIT ( oedit_autoarmor )
{
    ...
    {
        send_to_char
            ( " {cAutoArmor requires a level to be set on the armor first.{x\n\r",
              ch );
        return FALSE;
    }
...
/* ROM values end */
    ...
    if ( pMob->mprogs )
    {
        ...
        for ( cnt = 0, list = pMob->mprogs; list; list = list->next )
        {
            ...
            sprintf ( buf, "[%4d] %4ld %7s %s\n\r", cnt, list->vnum,
                      mprog_type_to_name ( list->trig_type ),
                      list->trig_phrase );
            send_to_char ( buf, ch );
            cnt++;
        }
...
MEDIT ( medit_position )
{
    ...
    switch ( arg[0] )
    {
        ...
        case 's':
            ...
            if ( ( value =
                   flag_value ( position_flags, argument ) ) == NO_FLAG )
                break;
...
void show_liqlist ( CHAR_DATA * ch )
{
    ...
    for ( liq = 0; liq_table[liq].liq_name != NULL; liq++ )
    {
        if ( ( liq % 21 ) == 0 )
            ...
            buffer_strcat ( buffer, "Name                 Color          Proof Full Thirst Food Ssize\n\r" );
...
void show_damlist ( CHAR_DATA * ch )
{
    ...
    for ( att = 0; attack_table[att].name != NULL; att++ )
    {
	    ...
	    buffer_strcat ( buffer, "Name                 Noun\n\r" );
...
MEDIT ( medit_addmprog )
{
    ...
    if ( ( code = get_mprog_index ( atol ( num ) ) ) == NULL )
    {
...
// sprintf(temp, "%d %d %d %d", ac_n, ac_n, ac_n, ac_x);
    ...
    medit_damdice ( ch, temp );
    bonus =
        ( pMob->level / 40 + 10 ) * ( ( pMob->level * 10 ) +
                                      ( pMob->level / 10 ) ) + 1;
    size = pMob->level / 3;
    if ( size < 2 )
        size = 2;
