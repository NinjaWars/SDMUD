...
CH_CMD ( do_bane )
{
    ...
    if ( arg[0] == '\0' )
    {
        ...
        act ( "$N says '{aI offer the following services:{x'", ch, NULL, mob,
              TO_CHAR );
        send_to_char ( "  align: Rhaelar's favor          20 gold\n\r", ch );
        send_to_char ( "  sanctuary: Rhaelar's protection 35 gold\n\r", ch );
        send_to_char ( "  voodoo: Remove voodoo curses   1 platinum\n\r",
                       ch );
        send_to_char ( " Type bane <type> to be cursed.\n\r", ch );
        return;
    }
    ...
    {
        act ( "$N says '{aType 'bane' for a list of spells.{x'", ch, NULL,
              mob, TO_CHAR );
        return;
    }
    ...
    {
        act ( "$N says '{aRhaelar does not protect the pure of heart!{x'", ch,
              NULL, mob, TO_CHAR );
        return;
    }
