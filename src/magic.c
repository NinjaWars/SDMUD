...
bool saves_dispel( int dis_level, int spell_level, int duration)
{
	...
	save = 0 + (spell_level - dis_level) * 2;
	save = URANGE( 5, save, 95 );
	return number_percent( )+30 < save;
}
...
bool check_dispel( int dis_level, CHAR_DATA *victim, int sn)
{
	...
	if (is_affected(victim, sn))
	{
		...
		if (!is_affected(victim, sn))			/* really gone */
		{
			if ( skill_table[sn].msg_off )
			{
				send_to_char( skill_table[sn].msg_off, victim );
...
CH_CMD ( do_cast )
{
    ...
    {
	send_to_char ("How exactly are you planning to {Gcast{x that {Gskill{x?!",ch);
	return;
    }
...
MAGIC ( spell_blindness )
{
    ...
    {
	send_to_char ( "You failed to blind them.\n\r", ch );
        return;
    }
...
MAGIC ( spell_cancellation )
{
    ...
    if ( check_dispel ( level, victim, skill_lookup ( "megafaerie fire" ) ) )
    {
        act ( "$n's huge outline fades.", victim, NULL, NULL, TO_ROOM );
        found = TRUE;
    }
...
MAGIC ( spell_cause_light )
{
    ...
    damage_old ( ch, victim, dice ( 1, 8 ) + level / 3, sn, DAM_HARM, TRUE, 0 );
    return;
}
...
MAGIC ( spell_cause_critical )
{
    ...
    damage_old ( ch, victim, dice ( 3, 8 ) + level - 6, sn, DAM_HARM, TRUE, 0 );
    return;
}
...
MAGIC ( spell_cause_serious )
{
    ...
    damage_old ( ch, victim, dice ( 2, 8 ) + level / 2, sn, DAM_HARM, TRUE, 0 );
    return;
}
    ...
    while ( level > 0 )
    {
              ...
              tmp_vict = next_vict )
        {
                 ...
                 tmp_vict != last_vict )
            {
                ...
                if ( saves_spell ( level, tmp_vict, DAM_LIGHTNING ) )
                    dam /= 3;
                damage_old ( ch, tmp_vict, dam, sn, DAM_LIGHTNING, TRUE, 0 );
                level -= 4;     /* decrement damage */
            }
        ...
        if ( !found )           /* no target found, hit the caster */
        {
            ...
            if ( saves_spell ( level, ch, DAM_LIGHTNING ) )
                dam /= 3;
            damage_old ( ch, ch, dam, sn, DAM_LIGHTNING, TRUE, 0 );
            level -= 4;         /* decrement damage */
            if ( ch == NULL )
                return;
        }
...
MAGIC ( spell_charm_person )
{
    ...
    {
        send_to_char
            ( "The fist clan does not allow charming in the city limits.\n\r",
              ch );
        return;
    }
...
MAGIC ( spell_colour_spray )
{
    ...
    damage_old ( ch, victim, dam, sn, DAM_LIGHT, TRUE, 0 );
    return;
}
...
*/
    ...
    if ( saves_spell ( level, victim, DAM_NEGATIVE ) )
        dam /= 2;
    damage_old ( ch, victim, dam, sn, DAM_NEGATIVE, TRUE, 0 );
    damage_old ( ch, victim, dam, sn, DAM_NEGATIVE, TRUE, 0 );
...
MAGIC ( spell_dispel_evil )
{
    ...
    if ( IS_GOOD ( victim ) )
    {
        act ( "The of of good protects $N.", ch, NULL, victim, TO_ROOM ); // Previously Sina
        return;
    }
...
MAGIC ( spell_dispel_good )
{
    ...
    if ( IS_EVIL ( victim ) )
    {
        act ( "The god of evil protects $N.", ch, NULL, victim, TO_ROOM ); // Previously Rhaelar
        return;
    }
...
MAGIC ( spell_dispel_magic )
{
    ...
    {
        act ( "$n is no longer surrounded by a fireshield.", victim, NULL, NULL, TO_ROOM );
        found = TRUE;
    }

    if ( check_dispel ( level, victim, skill_lookup ( "iceshield" ) ) )
    {
        act ( "$n is no longer surrounded by a iceshield.", victim, NULL, NULL, TO_ROOM );
        found = TRUE;
    }

    if ( check_dispel ( level, victim, skill_lookup ( "shockshield" ) ) )
    {
        act ( "$n is no longer surrounded by a shockshield.", victim, NULL, NULL, TO_ROOM );
        found = TRUE;
    }

    if ( check_dispel ( level, victim, skill_lookup ( "briarshield" ) ) )
    {
        act ( "$n is no longer surrounded by a fireshield.", victim, NULL, NULL, TO_ROOM );
        found = TRUE;
    }
    ...
    {
        act ( "$n is no longer surrounded by an acidshield.", victim, NULL, NULL, TO_ROOM );
        found = TRUE;
    }
        ...
        if ( vch->in_room == ch->in_room )
        {
            if ( vch != ch && !is_safe_spell ( ch, vch, TRUE ) )
            {
                ...
                if ( IS_AFFECTED ( vch, AFF_FLYING ) )
                    damage_old ( ch, vch, 0, sn, DAM_BASH, TRUE, 0 );
                else
...
//         !can_see_room ( ch, victim->in_room ) ||
    ...
    if ( IS_NPC ( victim ) && ( ch->pcdata->questmob == victim->pIndexData->vnum ) )
    {
        send_to_char ( "You can't gate to your questmob, you silly bear.\n\r",
                       ch );
        return;
    }
...
MAGIC ( spell_harm )
{
    ...
    if ( saves_spell ( level, victim, DAM_HARM ) )
        dam = UMIN ( 50, dam / 2 );
    dam = UMIN ( 100, dam );
    damage_old ( ch, victim, dam, sn, DAM_HARM, TRUE, 0 );
    return;
}
    ...
    for ( vch = ch->in_room->people; vch != NULL; vch = vch_next )
    {
                  ...
                  ( IS_EVIL ( ch ) && IS_GOOD ( vch ) ) )
        {
            if ( !is_safe_spell ( ch, vch, TRUE ) )
            {
                ...
                dam = dice ( level, 60 );
                damage_old ( ch, vch, dam, sn, DAM_ENERGY, TRUE, 0 );
            }
        }
...
MAGIC ( spell_magic_missile )
{
    ...
    damage_old ( ch, victim, dam, sn, DAM_ENERGY, TRUE, 0 );
    return;
}
...
MAGIC ( spell_summon )
{
    ...
    if ( strstr ( victim->in_room->area->builders, "Unlinked" ) )
    {
        send_to_char ( "You can't summon mobs from areas that aren't linked yet!\n\r",
                       ch );
        return;
    }
    ...
    if ( IS_NPC ( victim ) && ( ch->pcdata->questmob == victim->pIndexData->vnum ) )
    {
        send_to_char ( "You can't summon your questmob, you silly bear.\n\r",
                       ch );
        return;
    }
...
MAGIC ( spell_teleport )
{
    ...
    if ( strstr ( pRoomIndex->area->builders, "Unlinked" ) )
    {
        send_to_char ( "You failed.\n\r", ch );
        return;
    }
    ...
    while (IS_SET(pRoomIndex->room_flags,ROOM_NO_RECALL) || IS_SET(pRoomIndex->room_flags,ROOM_NEWBIES_ONLY) ||
    IS_SET(pRoomIndex->room_flags,ROOM_GODS_ONLY) || IS_SET(pRoomIndex->room_flags,ROOM_PRIVATE) ||
    IS_SET(pRoomIndex->room_flags,ROOM_SOLITARY) || IS_SET(pRoomIndex->room_flags,ROOM_IMP_ONLY) ||
    IS_SET(pRoomIndex->room_flags,ROOM_ARENA) || IS_SET(pRoomIndex->room_flags,ROOM_NEWBIES_ONLY));
/*
...
*/	
    ...
    {
	send_to_char ("You failed.\n\r", ch);
	send_to_char ("$n tries to teleport you elsewhere but failed.\n\r", victim);
	return;
    }
    ...
    act ( "$n appears.", victim, NULL, NULL, TO_ROOM );
    do_look ( victim, "auto" );
    return;
}
/*
...
MAGIC ( spell_word_of_recall )
{
    ...
    {
        act ( "$G refuses your request for recovery as you are still {Rviolent{x.", ch, NULL, NULL, TO_CHAR );
        return;
    }
...
MAGIC ( spell_acid_breath )
{
    ...
    if ( saves_spell ( level, victim, DAM_ACID ) )
    {
        ...
        damage_old ( ch, victim, dam / 2, sn, DAM_ACID, TRUE, 0 );
    }
    else
    {
    ...
    for ( vch = victim->in_room->people; vch != NULL; vch = vch_next )
    {
        ...
        if ( vch == victim )    /* full damage */
        {
            ...
            if ( saves_spell ( level, vch, DAM_FIRE ) )
            {
                ...
                damage_old ( ch, vch, dam / 2, sn, DAM_FIRE, TRUE, 0 );
            }
            else
            {
                fire_effect ( vch, level, dam, TARGET_CHAR );
                damage_old ( ch, vch, dam, sn, DAM_FIRE, TRUE, 0 );
            }
        }
        else                    /* partial damage */
        {
            if ( saves_spell ( level - 2, vch, DAM_FIRE ) )
            {
                ...
                damage_old ( ch, vch, dam / 4, sn, DAM_FIRE, TRUE, 0 );
            }
            else
            {
                fire_effect ( vch, level / 2, dam / 4, TARGET_CHAR );
    ...
    for ( vch = victim->in_room->people; vch != NULL; vch = vch_next )
    {
        ...
        if ( vch == victim )    /* full damage */
        {
            ...
            if ( saves_spell ( level, vch, DAM_COLD ) )
            {
                ...
                damage_old ( ch, vch, dam / 2, sn, DAM_COLD, TRUE, 0 );
            }
            else
            {
                cold_effect ( vch, level, dam, TARGET_CHAR );
                damage_old ( ch, vch, dam, sn, DAM_COLD, TRUE, 0 );
            }
        }
        else
        {
            if ( saves_spell ( level - 2, vch, DAM_COLD ) )
            {
                ...
                damage_old ( ch, vch, dam / 4, sn, DAM_COLD, TRUE, 0 );
            }
            else
            {
                cold_effect ( vch, level / 2, dam / 4, TARGET_CHAR );
...
MAGIC ( spell_gas_breath )
{
    ...
    for ( vch = ch->in_room->people; vch != NULL; vch = vch_next )
    {
        ...
        if ( saves_spell ( level, vch, DAM_POISON ) )
        {
            ...
            damage_old ( ch, vch, dam / 2, sn, DAM_POISON, TRUE, 0 );
        }
        else
        {
            poison_effect ( vch, level, dam, TARGET_CHAR );
            damage_old ( ch, vch, dam, sn, DAM_POISON, TRUE, 0 );
        }
    }
}
...
MAGIC ( spell_lightning_breath )
{
    ...
    if ( saves_spell ( level, victim, DAM_LIGHTNING ) )
    {
        ...
        damage_old ( ch, victim, dam / 2, sn, DAM_LIGHTNING, TRUE, 0 );
    }
    else
    {
...
MAGIC ( spell_general_purpose )
{
    ...
    damage_old ( ch, victim, dam, sn, DAM_PIERCE, TRUE, 0 );
    return;
}
...
//         !can_see_room ( ch, victim->in_room ) ||	 
         ...
         IS_SET ( ch->in_room->room_flags, ROOM_NO_RECALL )
         //|| (IS_NPC(victim) && is_gqmob(ch, victim->pIndexData->vnum) != -1) 
         || victim->level >= level + 10 || ( is_clan ( ch ) && ( is_clan ( victim ) && ( ( !is_same_clan ( ch, victim ) && ( clan_table[ch->clan].pkill ) && ( clan_table[victim->clan].pkill ) ) || clan_table[victim->clan].independent ) ) ) || ( !IS_NPC ( victim ) && victim->level >= LEVEL_ANCIENT )  /* NOT trust */
         || ( IS_NPC ( victim ) && IS_SET ( victim->imm_flags, IMM_SUMMON ) )
         || ( IS_NPC ( victim ) &&
              saves_spell ( level, victim, DAM_OTHER ) ) )
    {
    ...
    if ( strstr ( victim->in_room->area->builders, "Unlinked" ) )
    {
        send_to_char ( "You can't shadow walk to areas that aren't linked yet!\n\r",
                       ch );
        return;
    }
    ...
    if ( IS_NPC ( victim ) && ( ch->pcdata->questmob == victim->pIndexData->vnum ) )
    {
        send_to_char ( "You can't shadow walk to your questmob you silly bear.\n\r",
                       ch );
        return;
    }
    ...
    act ( "$n vanishes into the {Dshadow{x plane.", ch, NULL, NULL, TO_ROOM );
    send_to_char ( "You step into the {Dshadow{x plane.\n\r", ch );
    char_from_room ( ch );
    char_to_room ( ch, victim->in_room );

    act ( "$n has arrived through the {Dshadow{x plane.", ch, NULL, NULL, TO_ROOM );
    do_look ( ch, "auto" );

    if ( gate_pet )
    {
        act ( "$n vanishes into the {Dshadow{x plane.", ch->pet, NULL, NULL,
              TO_ROOM );
        send_to_char ( "You step into the {Dshadow{x plane.\n\r", ch->pet );
        char_from_room ( ch->pet );
        char_to_room ( ch->pet, victim->in_room );
        act ( "$n has arrived through the {Dshadow{x plane.", ch->pet, NULL, NULL,
              TO_ROOM );
        do_look ( ch->pet, "auto" );
    }
}
...
void say_spell( CHAR_DATA *ch, int sn )
{
	...
	for ( pName = skill_table[sn].name; *pName != '\0'; pName += length )
	{
		for ( iSyl = 0; (length = strlen(syl_table[iSyl].oldsyl)) != 0; iSyl++ )
		{
			if ( !str_prefix( syl_table[iSyl].oldsyl, pName ) )
			{
