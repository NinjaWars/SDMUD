...
#define L_SUP (MAX_LEVEL - 3) /* if not already defined */

BOARD_DATA boards[MAX_BOARD] =
{
{ "Announce", 	"Announcements from Immortals",  	 0,     L_SUP, "all", DEF_NORMAL, 365, NULL, FALSE },
{ "Changes", 	"Info specifically about changes",  	 0,     L_SUP, "all", DEF_NORMAL, 365, NULL, FALSE },
{ "General",  	"General discussion",           	 0,     1,     "all", DEF_INCLUDE, 90, NULL, FALSE },
{ "Clan",  	"Clan discussion",           	 	 0,     1,     "all", DEF_NORMAL, 90, NULL, FALSE },
{ "Ideas",	"Suggestions for improvement",	 	 0,     1,     "all", DEF_INCLUDE, 365, NULL, FALSE }, 
{ "Jokes",	"Funny stuff",			 	 0,     1,     "all", DEF_NORMAL, 90, NULL, FALSE }, 
{ "Market",	"Sell stuff, buy stuff",	 	 0,     1,     "all", DEF_NORMAL, 90, NULL, FALSE }, 
{ "Petition",	"Pfile weirdness and oddities",	 	 0,     1,     "Rhaelar", DEF_ONLY, 90, NULL, FALSE },
{ "Personal", 	"Personal messages",			 0,     1,     "all", DEF_EXCLUDE, 365, NULL, FALSE },
{ "RP",		"Role-playing notes",			 0,     1,     "all", DEF_NORMAL, 90, NULL, FALSE }, 
{ "Quests", 	"Quest announcements/discussion",	 0,     1,     "all", DEF_NORMAL, 90, NULL, FALSE },
{ "Weddings", 	"Wedding announcements",		 0,     LEVEL_IMMORTAL,     "all", DEF_INCLUDE, 365, NULL, FALSE },
{ "Bugs",	"Report bugs and typos",			 	 0,     1,     "Rhaelar", DEF_ONLY, 365, NULL, FALSE },
{ "Council", 	"Immortal Discussion", LEVEL_IMMORTAL,     LEVEL_IMMORTAL, "imm", DEF_ONLY, 1825, NULL, FALSE },
{ "Permissions", "Special permissions for players", LEVEL_IMMORTAL,     LEVEL_IMMORTAL, "imm", DEF_NORMAL, 1825, NULL, FALSE },
{ "Punishments", "Punishment Announcements", LEVEL_IMMORTAL,     LEVEL_IMMORTAL, "imm", DEF_NORMAL, 1825, NULL, FALSE },
{ "Gods", 	"Gods Discussion", 	L_SUP,     L_SUP, "gods", DEF_ONLY, 1825, NULL, FALSE },
{ "Mud", 	"Announcements from the MUD", 216, 216, "all", DEF_EXCLUDE, 30, NULL, FALSE }
};
    ...
    for ( ; ; )
    {
        ...
        if (pnote->expire < current_time)
        {
			...
			else
			{
				...
				new_fclose (fp_archive); /* it might be more efficient to close this later */
			}
...
bool is_note_to (CHAR_DATA *ch, NOTE_DATA *note)
{
	...
	if (IS_IMMORTAL(ch) && ( 
		...
		is_full_name ("immies", note->to_list) ||
...
static void do_nwrite (CHAR_DATA *ch, char *argument)
{
	...
	if (get_trust(ch) < ch->pcdata->board->write_level)
	{
        ...
        if ( !IS_NPC ( ch ) && IS_SET ( class_table[ch->class].tier, TIER_01 ) && ch->level < 20 )
	{
	   send_to_char ( "You can't write notes just yet.\n\r", ch );
	   return;
	}        
	...
	{
		send_to_char ( "\n\r{YYour status has been set to AFK while writing the note.{x\n\r", ch );
		send_to_char ( "{YYou will not see any MUD output while you are writing the note (like quiet mode).{x\n\r", ch );
		send_to_char ( "{YIf you receive tells, they will be replayed to you when you are finished.{x\n\r\n\r", ch );
...
//	act ( GREEN "$n starts writing a note." NO_COLOR , ch, NULL, NULL, TO_ROOM);
	...
	if (!ch->pcdata->in_progress->text) /* Are we continuing an old note or not? */
	{
		...
		case DEF_ONLY:
			sprintf (buf, "You may only write notes to \"" BOLD "%s" NO_COLOR "\" on this board. Hit return.", 
			ch->pcdata->board->names);
			break;
		}			
...
{
	int count= 0, show = 0, num = 0, has_shown = 0, visible_note = 0;
	time_t last_note;
	NOTE_DATA *p;
	BUFFER2 *notebuf;
	char buf[MSL];
		        
	notebuf = buffer_new ( 16000 );
	...
	send_to_char (BOLD "Notes on this board:" NO_COLOR "\n\r"
	              RED " Num>   Date                       Author          Subject" NO_COLOR "\n\r",ch);
/*	sprintf ( result, BOLD "Notes on this board:" NO_COLOR "\n\r"
 	              RED "Num >   Date                       Author          Subject" NO_COLOR "\n\r" ); */
	//add_buf ( output, buf );  
	...
	for (p = ch->pcdata->board->note_first; p; p = p->next)
	{
		...
		if (is_note_to(ch,p))
		{
			has_shown++; /* note that we want to see X VISIBLE note, not just last X 27, 15, 0*/
			if (!show || ((count-show) < has_shown))
			{
				sprintf (buf, BOLD "%4d" NO_COLOR ">" /*BLUE BOLD*/ "{Y%c" /*NO_COLOR YELLOW BOLD*/ "  {D%-27s" "{w%-15s" NO_COLOR YELLOW " %s" NO_COLOR " \n\r",               
					       num,
				               last_note < p->date_stamp ? '*' : ' ',               
					       p->date, p->sender, p->subject);
				buffer_strcat(notebuf, buf);
				++visible_note;
/* 
 * I would really like to use a buffer for scrolling, but this causes a buffer overflow
 * when there are more than 200 notes on the board. We really need this fixed! -Rhaelar
 */
//				add_buf(output,buf);
			}
		}
	...
	if ( visible_note <= 0 )
	{
	       sprintf ( buf, "   {DThere are no notes visible to you on this board.{x\n\r" );
	       buffer_strcat ( notebuf, buf );
	}	
/*      page_to_char ( buf_string ( output ), ch );
      free_buf ( output ); 
//       page_to_char ( result, ch ); */
      page_to_char ( buffer_string ( notebuf ), ch );
//      page_to_char ( notebuf, ch );
      buffer_free ( notebuf );
}
...
void do_board (CHAR_DATA *ch, char *argument)
{
	...
	if (!argument[0]) /* show boards */
	{
		...
		count = 1;
		send_to_char (BOLD RED "Num         Name  Total Unread Description" NO_COLOR "\n\r"
		              RED "=== ============ ====== ====== ===========" NO_COLOR "\n\r",ch);
		for (i = 0; i < MAX_BOARD; i++)
		{
			total = total_notes (ch,&boards[i]); /* how many total notes? */
			unread = unread_notes (ch,&boards[i]); /* how many unread notes? */
			if (unread != BOARD_NOACCESS)
			{ 
				sprintf (buf, BOLD "%2d" NO_COLOR "> " GREEN BOLD "%12s" NO_COLOR " [" GREEN "%4d" NO_COLOR "] " "[%s%4d" NO_COLOR "] " YELLOW "%s" NO_COLOR "\n\r", 
				                   count, boards[i].short_name, total, unread ? RED : GREEN, 
...
void handle_con_note_to (DESCRIPTOR_DATA *d, char * argument)
{
	...
	switch (ch->pcdata->board->force_type)
	{
		...
		case DEF_INCLUDE: /* forced default */
			...
			break;
		case DEF_ONLY:
				sprintf (buf, "%s", ch->pcdata->board->names);
				ch->pcdata->in_progress->to_list = str_dup(buf);
				write_to_buffer (d, "Note will be sent to the recipient specified by this board.\n\r", 0);
			break;
...
void handle_con_note_subject (DESCRIPTOR_DATA *d, char * argument)
{
	...
	{
		write_to_buffer (d, "No, no. This is just the Subject. You're not writing the note yet. Shorten it.\n\r",0);
	}
	else
	...
	CHAR_DATA *ch = d->character;
		...
		switch (tolower(argument[0]))
		{
			...
			case 'p': /* post note */
				...
				d->connected = CON_PLAYING;
				/* remove AFK status */
				send_to_char ( "\n\r{YAFK status removed.{x\n\r", ch );
				REMOVE_BIT ( ch->comm, COMM_AFK );
				do_replay ( ch, "" );
//				act (BOLD GREEN "$n finishes $s note." NO_COLOR , ch, NULL, NULL, TO_ROOM);
			        act ( "$n finishes $s note.", ch, NULL, NULL, TO_ROOM );
				char buf[MAX_STRING_LENGTH];
			        sprintf ( buf,
			                  "{R[{WNote{R] {D%s{x just posted a note called {W'{c%s{W' {xon the {g%s{x board!\n\r",
			                  ch->pcdata->cname, ch->pcdata->in_progress->subject, ch->pcdata->board->short_name );
			        for ( d = descriptor_list; d; d = d->next )
			        {
			            if ( d->connected == CON_PLAYING &&
			                 is_note_to ( d->character, ch->pcdata->in_progress ) )
			            {
				...
				ch->pcdata->in_progress = NULL;
/*
				for (i = 0; i < length(note.to); ++i)
				{
				  if (note.to[i] == "all") {
				    do_gmessage("{RA n{rote {Rto a{rll{r w{Ras {rjus{Rt p{ros{Rte{rd. C{Rhe{rck yo{Rur {rbo{Ra{rrds.{x\n\r");
				  }
				  else {
				    for (plr = plrlst.next; plr != NULL; ++plr) 
				    {
				      if (plr.name == note.to[i]) {
				        write_to_plr("{YA n{yote {Yad{yress{Yed {yto y{You {ywas {Yju{yst pos{Yte{yd. {YCh{yeck yo{Yu{yr bo{Yar{yds.{x\n\r");
				      }
				    }
				  }
				}
*/
			...
			case 'f':
				...
				REMOVE_BIT ( ch->comm, COMM_AFK );
				do_replay ( ch, "" );
//				act ( GREEN "$n aborts $s note." NO_COLOR , ch, NULL, NULL, TO_ROOM);;
			        act ( "$n aborts $s note.", ch, NULL, NULL, TO_ROOM );
				break;
