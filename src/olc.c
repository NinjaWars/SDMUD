...
CH_CMD ( do_redit )
{
    ...
    if ( !str_cmp ( arg1, "reset" ) )
    {
        ...
        {
            send_to_char
                ( "Insuficiente seguridad para modificar cuartos.\n\r", ch );
            return;
        }
    ...
    {
        pRoom2 = get_room_index ( atol ( arg1 ) );
        ...
        {
            send_to_char
                ( "Insuficiente seguridad para editar cuarto, o cuarto inexistente.\n\r",
                  ch );
            return;
        }
    }
...
CH_CMD ( do_oedit )
{
    ...
    if ( is_number ( arg1 ) )
    {
        ...
        {
            send_to_char
                ( "Insuficiente seguridad para modificar objetos.\n\r", ch );
            return;
        }
...
CH_CMD ( do_medit )
{
    ...
    else
    {
        if ( !str_cmp ( arg1, "create" ) )
        {
            ...
            {
                send_to_char
                    ( "Insuficiente seguridad para modificar mobs.\n\r", ch );
                return;
            }
    ...
    for ( pReset = pRoom->reset_first; pReset; pReset = pReset->next )
    {
        ...
        switch ( pReset->command )
        {
            ...
            case 'M':
                ...
                pMob = pMobIndex;
                sprintf ( buf,
                          "M[%ld] %-13.13s in room             R[%ld] %2d-%2d %-15.15s\n\r",
            ...
            case 'P':
                ...
                sprintf ( buf,
                          "O[%ld] %-13.13s inside              O[%ld] %2d-%2d %-15.15s\n\r",
            ...
            case 'E':
                ...
                {
                    sprintf ( buf,
                              "O[%ld] %-13.13s in the inventory of S[%ld]       %-15.15s\n\r",
                              pReset->arg1, pObj->short_descr, pMob->vnum,
                              pMob->short_descr );
                }
                else
                    sprintf ( buf,
                              "O[%ld] %-13.13s %-19.19s M[%ld]       %-15.15s\n\r",
            ...
            case 'D':
                ...
                sprintf ( buf, "R[%ld] %s door of %-19.19s reset to %s\n\r",
            ...
            case 'R':
                ...
                sprintf ( buf, "R[%ld] Exits are randomized in %s\n\r",
...
CH_CMD ( do_resets )
{
    ...
    if ( is_number ( arg1 ) )
    {
                 ...
                 ( !str_cmp ( arg2, "obj" ) && is_number ( arg3 ) ) )
        {
            ...
            if ( !str_cmp ( arg2, "mob" ) )
            {
                if ( get_mob_index ( is_number ( arg3 ) ? atol ( arg3 ) : 1 )
                     == NULL )
                {
            ...
            if ( !str_cmp ( arg2, "obj" ) )
            {
                ...
                if ( !str_prefix ( arg4, "inside" ) )
                {
                    ...
                    temp =
                        get_obj_index ( is_number ( arg5 ) ? atol ( arg5 ) :
                                        1 );
...
CH_CMD ( do_alist )
{
	...
	for (pArea = area_first; pArea != NULL; pArea = pArea->next)areas[i++] = pArea;

	qsort(areas, i, sizeof(pArea), compare_area);

	while (--i >= 0)
	{
		sprintf(buf, "[%3d] %-25s [%-5ld-%5ld] %-13.13s [%1d] [%-10s]\n\r",
		areas[i]->vnum,      areas[i]->name,       areas[i]->min_vnum,
		areas[i]->max_vnum,  areas[i]->file_name,  areas[i]->security,
		areas[i]->builders);
		strcat(result, buf);
	}
		sprintf(buf, "\n\rTotal areas: %d (Next free: %d)\n\r", (top_area - 1), top_area );
		strcat(result, buf);
		/*stc( result, ch );*/
