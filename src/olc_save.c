...
void save_area_list (  )
{
    ...
    else
    {
         ...
         */
        fprintf ( fp, "../data/helps/help.are\n" );
        fprintf ( fp, "../data/helps/rot.are\n" );    /* ROM OLC */
        fprintf ( fp, "../data/helps/group.are\n" );  /* ROM OLC */
        fprintf ( fp, "../data/helps/olc.hlp\n" );
...
void save_mobile ( FILE * fp, MOB_INDEX_DATA * pMobIndex )
{
    ...
    fprintf ( fp, "%ldd%ld+%ld ", pMobIndex->damage[DICE_NUMBER],
    ...
    if ( ( temp = DIF ( race_table[race].shd, pMobIndex->shielded_by ) ) )
        fprintf ( fp, "F shd %s\n", fwrite_flag ( temp, buf ) );
...
/*                   fwrite_flag( pObjIndex->value[3], buf ) ); */
        ...
        case ITEM_FOOD:
            fprintf ( fp, "%ld %ld 0 %s 0\n", pObjIndex->value[0],
                      pObjIndex->value[1], fwrite_flag ( pObjIndex->value[3],
                                                         buf ) );
            break;
        ...
        case ITEM_FURNITURE:
            fprintf ( fp, "%ld %ld %s %ld %ld\n", pObjIndex->value[0],
                      pObjIndex->value[1], fwrite_flag ( pObjIndex->value[2],
                                                         buf ),
                      pObjIndex->value[3], pObjIndex->value[4] );
            break;
            ...
            if ( pRoomIndex->area == pArea )
            {
                for ( door = 0; door < MAX_DIR; door++ )
                {
...
#if defined( VERBOSE )
                        fprintf ( fp,
                                  "D 0 %ld %d %d The %s door of %s is %s\n",
                    ...
                    fprintf ( fp, "D 0 %ld %d %d\n", pRoomIndex->vnum,
                              pExit->orig_door, IS_SET ( pExit->rs_flags,
                                                         EX_LOCKED ) ? 2 :
                              1 );
#endif
                }
                      ...
                      pReset = pReset->next )
                {
                    switch ( pReset->command )
                    {
                        ...
                        case 'M':
                            ...
                            fprintf ( fp, "M 0 %ld %d %ld %d Load %s\n",
                        ...
                        case 'O':
                            ...
                            fprintf ( fp, "O 0 %ld 0 %ld %s loaded to %s\n",
