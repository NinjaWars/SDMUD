...
void load_mobiles ( FILE * fp )
{
    ...
    for ( ;; )
    {
        ...
        pMobIndex->act2 =
            fread_flag ( fp ) | race_table[pMobIndex->race].act2;
        pMobIndex->affected_by =
            fread_flag ( fp ) | race_table[pMobIndex->race].aff;
        pMobIndex->shielded_by =
            fread_flag ( fp ) | race_table[pMobIndex->race].shd;
                ...
                switch ( pReset->command )
                {
                    case 'M':
                        if ( !( pMob = get_mob_index ( pReset->arg1 ) ) )
                            bug ( "Convert_objects: 'M': bad vnum %ld.",
                                  pReset->arg1 );
                        break;
                    ...
                    case 'O':
                        ...
                        {
                            bug ( "Convert_objects: 'O': bad vnum %ld.",
                                  pReset->arg1 );
                            break;
                        }
                    ...
                    case 'P':
                    {
                        ...
                        if ( !( pObj = get_obj_index ( pReset->arg1 ) ) )
                        {
                            bug ( "Convert_objects: 'P': bad vnum %ld.",
                                  pReset->arg1 );
                            break;
                        }
                        ...
                        pObj->level =
                            pObj->level <
                            1 ? pObjTo->level : UMIN ( pObj->level,
                                                       pObjTo->level );
                    }
                        break;
                    ...
                    case 'E':
                        if ( !( pObj = get_obj_index ( pReset->arg1 ) ) )
                        {
                            bug ( "Convert_objects: 'E' or 'G': bad vnum %ld.", pReset->arg1 );
                            break;
                        }

                        if ( !pMob )
                        {
                            bug ( "Convert_objects: 'E' or 'G': null mob for vnum %ld.", pReset->arg1 );
                            break;
                        }
