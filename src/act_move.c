...
void announce_move( CHAR_DATA *ch, char *message, ROOM_INDEX_DATA *room )
{
	CHAR_DATA *to;
	...
	{
		if ( (ch != to) &&						/* don't look at yourself */
			( can_see_sneak( to, ch ) ||		/* can see (sneaking too) */
		   ( !IS_AFFECTED(ch, AFF_SNEAK)
		&&  (ch->invis_level <= LEVEL_HERO)		/* or see Someone leave   */
		&&  (ch->ghost_level <= LEVEL_HERO)) ))
			act( message, ch, NULL, to, TO_VICT);
	}
}
...
void move_char ( CHAR_DATA * ch, int door, bool follow, bool quiet )
{
         ...
         !can_see_room ( ch, pexit->u1.to_room ) )
    {
        if ( !quiet )
			...
			{
				case 1:  send_to_char("You stumble all over yourself trying to do in that direction.\n\r",ch);
					break;	
				case 2:  send_to_char("You take a running start and slam head first into a wall.\n\r",ch);
					break;	
			    	case 3:	send_to_char("Alas, you cannot go that way.\n\r",ch);
					break;	
				case 4:	send_to_char("THUD! Your sore face suggests there wasn't a secret passage there afterall.\n\r",ch);
					break;	
				case 5: send_to_char("WHAM! Face first into the wall! Good one spaz.\n\r",ch);
					break;	
				case 6: send_to_char("You huff and puff trying to go that way, but, alas, you can't!\n\r",ch);
					break;	
				default: send_to_char("Alas, you cannot go that way.\n\r",ch);
					break;
			}
        return;
    }
...
/*
 * I know this is very poor, but I couldn't get the room_is_private function fixed properly
 * - right now it just says the room is private even if its not private and just got one owner or more,
 * and I had to make something to allow people to bring in guests into their houses. (Which have them as owners).
 * -Rhaelar
 */
    if ( !is_room_owner ( ch, to_room ) && IS_SET ( to_room->room_flags, ROOM_HOME ) && IS_SET ( to_room->room_flags, ROOM_PRIVATE ) )
    {
        if ( !quiet )
    ...
    if ( !is_room_owner ( ch, to_room ) && !IS_SET ( to_room->room_flags, ROOM_HOME ) && room_is_private ( ch, to_room ) ) 
    {
        if ( !quiet )
            send_to_char ( "That room is private right now.\n\r", ch );
        return;
...
*/
	...
	{
		sprintf (buf, "$n stumbles off drunkenly on %s way %s.", his_her[URANGE(0,ch->sex,2)],dir_name[door]);
	}
	...
	else if ( IS_AFFECTED ( ch, AFF_FLYING ) )
	{
		...
		{
			sprintf(buf, "$n flaps %s wings and flies %s%s.", his_her[URANGE(0,ch->sex,2)], dir_name[door], IS_AFFECTED(ch, AFF_SNEAK) ? ", trying to be quiet" : "");
		}
		else
		{
			sprintf(buf, "$n flies %s%s.", dir_name[door], IS_AFFECTED(ch, AFF_SNEAK) ? ", hoping nobody notices" : "");
		}
	}
	else if ( ch->hit < ch->max_hit/2 )
	{
		sprintf(buf, "$n crawls %s%s.", dir_name[door],IS_AFFECTED(ch, AFF_SNEAK) ? ", sneaking like a coward" : "");
	}
	else
	{
		sprintf(buf, "$n leaves %s%s.", dir_name[door],IS_AFFECTED(ch, AFF_SNEAK) ? ", hoping nobody notices" : "");
	}

	if (!quiet)
	{
		announce_move( ch, buf, in_room);
	}
    char_from_room( ch );
    char_to_room( ch, to_room );
    if      ( door == 0 ) {revdoor = 2;sprintf(dirbuf,"the {Bs{bouth{x");}
    else if ( door == 1 ) {revdoor = 3;sprintf(dirbuf,"the {Cw{cest{x");}
    else if ( door == 2 ) {revdoor = 0;sprintf(dirbuf,"the {Rn{rorth{x");}
    else if ( door == 3 ) {revdoor = 1;sprintf(dirbuf,"the {Ge{gast{x");}
    else if ( door == 4 ) {revdoor = 5;sprintf(dirbuf,"{Wb{Delow{x");}
    else                  {revdoor = 4;sprintf(dirbuf,"{Ma{mbove{x");}
	...
	if (!IS_NPC(ch) && ch->pcdata->condition[COND_DRUNK] > 10)
	{
		sprintf (buf, "$n stumbles in drunkenly from %s.", dirbuf);
	}
	else if ( IS_AFFECTED ( ch, AFF_FLYING ) )
		if ( IS_SET( ch->parts, PART_WINGS ))
			sprintf(buf, "$n glides in from %s%s.", dirbuf, 
				IS_AFFECTED(ch, AFF_SNEAK) ? ", landing very quietly" : "");
		else sprintf(buf, "$n flies in from %s%s.", dirbuf,
				IS_AFFECTED(ch, AFF_SNEAK) ? " and hovers silently" : "");
	else if ( ch->hit < ch->max_hit/2 )
		sprintf(buf, "$n crawls in from %s%s.", dirbuf,
			IS_AFFECTED(ch, AFF_SNEAK) ? ", hoping nobody notices" : "");
	else sprintf(buf, "$n arrives from %s%s.", dirbuf,
			IS_AFFECTED(ch, AFF_SNEAK) ? ", staying out of everyone's way" : "");

	if (!quiet)
		announce_move( ch, buf, to_room );
...
CH_CMD ( do_stand )
{
    ...
    switch ( ch->position )
    {
        case POS_SLEEPING:
            ...
            if ( IS_AFFECTED ( ch, AFF_STRANGLE ) )
            {
                send_to_char ( "You can't wake up!\n\r", ch );
                return;
            }
...
{
    send_to_char( "{cYou go to sleep.\n\r{c", ch );
    act( "{c$n goes to sleep.{c", ch, NULL, NULL, TO_ROOM );
...
else  /* find an object and sleep on it */
{
    ...
    if (obj == NULL)
    {
		...
		if (obj == NULL)
		{
		/*send_to_char( "{cYou don't see that here.\n\r{c",ch);*/
		stc("You cant find it.\n\r",ch);
		return;
		}
    }
...
/*    REMOVE_BIT ( ch->shielded_by, SHD_SWALK ); */
    send_to_char ( "All invisible/hiding affects removed.\n\r", ch );
    return;
}
...
//        if ( ( location = get_room_index ( ROOM_VNUM_TEMPLE ) ) == NULL )
    ...
    {
	send_to_char ( "You're already here!", ch );
        return;
    }

    if ( ch->in_room->vnum == ROOM_VNUM_MS )
    {
	send_to_char ( "You're already here!", ch );
        return;
    }
    ...
    if ( ( victim = ch->fighting ) != NULL )
    {
        ...
        if ( number_percent (  ) < 70 * skill / 100 )
        {
