...
/* See if the character is a "real" PK'er (not in noclan, exile, PKNewbies or NPK clans)
   The function is not dynamic, needs to be updated every time the clan list is changed -Rhaelar 11'2004 */
bool is_real_pkiller ( CHAR_DATA * ch )
{
   if ( !IS_NPC ( ch ) )
   {
    if ( ( ch->clan == 1 ||
...
int get_trust ( CHAR_DATA * ch )
{
    ...
    if ( ch->trust != 0 && IS_SET ( ch->act2, PLR2_TRUE_TRUST ) )
        return ch->trust;

    if ( IS_NPC ( ch ) && ch->level >= LEVEL_ANCIENT )
        return LEVEL_ANCIENT - 1;
    else
        return ch->level;
}
...
void char_from_room ( CHAR_DATA * ch )
{
    ...
    if ( is_affected ( ch, gsn_ambush ) )
    {
        ...
        send_to_char ( "You leave your hiding place.\n\r", ch );
    }
    return;
}
...
/*        if ( ch->alignment < 0 )
	...
	if ( ( room = get_room_index ( hometown_table[ch->hometown].recall ) ) != NULL )
		char_to_room( ch, room );
    ...
    if ( !IS_NPC ( ch ) )
    {
        ...
        ++ch->in_room->area->nplayer;
        setbit ( ch->pcdata->explored, pRoomIndex->vnum );
    }
 ...
 */
    ...
    if ( debug_mode )
    {
       sprintf ( debug, "[extract_char] Now nuking %s's pets", ch->name );    
       debug_log ( debug );
    }
    nuke_pets ( ch );
    ...
    if ( debug_mode )
    {
    	sprintf ( debug, "[extract_char] Now stopping %s's followers", ch->name );    
    	debug_log ( debug );
    }
	die_follower ( ch );
    ...
    for ( obj = ch->carrying; obj != NULL; obj = obj_next )
    {
    	...
    	if ( debug_mode )
	{
	   sprintf ( debug, "[extract_char] Now extracting obj '%s' from %s", obj->name, ch->name );    
    	   debug_log ( debug );
	}
        extract_obj ( obj );
    }
...
/*        if ( ( clan_table[ch->clan].hall == ROOM_VNUM_ALTAR ) &&
    ...
    {
       if ( IS_NPC ( ch ) )
       sprintf ( debug, "[extract_char] Now freeing %s's character [%ld]", ch->name, ch->pIndexData->vnum );
       else
       sprintf ( debug, "[extract_char] Now freeing %s's character", ch->name );       
       debug_log ( debug );
    }
    free_char ( ch );
    return;
...
CHAR_DATA *get_char_mortal ( CHAR_DATA * ch, char *argument )
{
    ...
    for ( wch = char_list; wch != NULL; wch = wch->next )
    {
        if ( !str_cmp ( arg, wch->name ) && !IS_NPC ( wch ) &&
             !IS_IMMORTAL ( wch ) )
            return wch;
    }
...
/*        if ( ch->platinum > 50000 )
	  ...
	  if ( ch->platinum > 50000 )
	  {
	    send_to_char ( "You have a terribly high amount of platinum held. Put some of it in the bank!\n\rPlatinum gain removed.", ch );
	    ch->platinum = 50000;
	  }
        return;
    }
...
bool is_room_owner ( CHAR_DATA * ch, ROOM_INDEX_DATA * room )
{
    ...
    if ( IS_MARRIED ( ch ) &&
         is_name ( ch->pcdata->spouse, room->owner ) )
        return TRUE;
...
bool room_is_private ( CHAR_DATA * ch, ROOM_INDEX_DATA * pRoomIndex )
{
    ...
    if ( IS_SET ( pRoomIndex->room_flags, ROOM_LOCKED ) && IS_NPC ( ch ) &&
         !IS_SET ( ch->act, ACT_KEY ) )
        return TRUE;
...
bool can_see_room ( CHAR_DATA * ch, ROOM_INDEX_DATA * pRoomIndex )
{
    ...
    if ( IS_SET ( pRoomIndex->room_flags, ROOM_NEWBIES_ONLY ) && ch->level > 9
         && !IS_HERO ( ch ) )
        return FALSE;
...
/* RT changed so that WIZ_INVIS has levels */
   ...
   if (!IS_NPC (ch) && !IS_NPC (victim))
   {
    ...
    if ( IS_MARRIED ( ch ) && IS_MARRIED ( victim ) &&
         !str_cmp ( ch->pcdata->spouse, victim->name ) &&
...
*/
    ...
    if ( is_affected ( victim, gsn_ambush ) )
        return FALSE;
/*
    if ( IS_SHIELDED ( victim, SHD_SWALK ) )
        return FALSE;
*/
    /* sneaking */
...
char *affect_bit_name ( int vector )
{
    ...
    if ( vector & AFF_DETECT_SCRY )
        strcat ( buf, " detect_scry" );
...
char *shield_bit_name ( int vector )
{
    ...
    if ( vector & SHD_POISON )
        strcat ( buf, " poisonshield" );
    if ( vector & SHD_BRIAR )
        strcat ( buf, " briarshield" );
...
char *act_bit_name ( int act_flags )
{
    ...
    {
        strcat ( buf, " player" );
        if ( act_flags & PLR_NOCOLOUR )
            strcat ( buf, " no_colour" );
...
char *act2_bit_name ( int act2_flags )
{
    ...
    if ( act2_flags & ACT2_FORGER )
        strcat ( buf, " forger" );
    if ( act2_flags & ACT2_DEALER )
        strcat ( buf, " dealer" );
...
/* return ascii name of an act vector */
char *plr2_bit_name ( int act2_flags )
{
    static char buf[512];

    buf[0] = '\0';
    if ( act2_flags & PLR2_CHALLENGER )
        strcat ( buf, "challenger" );
    if ( act2_flags & PLR2_CHALLENGED )
        strcat ( buf, "challenged" );
    if ( act2_flags & PLR2_CONSENT )
        strcat ( buf, "consent" );
    if ( act2_flags & PLR2_WIPED )
        strcat ( buf, "wiped" );
    if ( act2_flags & PLR2_TRUE_TRUST )
        strcat ( buf, "true_trust" );
    if ( act2_flags & PLR2_PKVIOLENT )
        strcat ( buf, "pkviolent" );
    if ( act2_flags & PLR2_PKSAFE )
        strcat ( buf, "pksafe" );
    if ( act2_flags & PLR2_HELPER )
        strcat ( buf, "helper" );
//    if (act2_flags & PLR2_BLAH        ) strcat(buf, " blah");
    return ( buf[0] != '\0' ) ? buf + 1 : "none";
}
...
char *comm_bit_name ( int comm_flags )
{
    ...
    if ( comm_flags & COMM_NOSOCIAL )
        strcat ( buf, " nosocial" );
    if ( comm_flags & COMM_NORACE )
        strcat ( buf, "norace" );
    if ( comm_flags & COMM_CURSE )
        strcat ( buf, "curse" );
    if ( comm_flags & COMM_NOSAY )
        strcat ( buf, "nosay" );
/*    if ( comm_flags & COMM_NOCHEER )
        strcat ( buf, "nocheer" ); */
...
char *wear_bit_name ( int wear_flags )
{
    ...
    if ( wear_flags & ITEM_WEAR_TATTOO )
        strcat ( buf, " tattoo" );
    if ( wear_flags & ITEM_WEAR_AURA )
        strcat ( buf, " aura" );
    if ( wear_flags & ITEM_WEAR_EAR )
        strcat ( buf, " ear" );
    if ( wear_flags & ITEM_WEAR_CTATTOO )
...
bool can_see_sneak( CHAR_DATA *ch, CHAR_DATA *victim )
{
	if ( ch == victim )
	return TRUE;

	if (ch->trust > victim->invis_level && ch->trust > victim->incog_level)
	    return TRUE;
	...
	if (IS_SET(ch->act, PLR_HOLYLIGHT))  	/* Holy Light works */
	return TRUE;
	...
	if (victim->fighting != NULL)		  	/* Not hiding when fighting */
	return TRUE;
	...
	{
	int chance;
	chance = get_skill(victim,gsn_sneak) * 3/2;
	chance += get_curr_stat(victim,STAT_DEX) * 3/2;
 	chance -= get_curr_stat(ch,STAT_INT) * 2;
	chance -= ch->level - victim->level * 3/2;
...
bool is_banned( DESCRIPTOR_DATA *d )
{
  ...
  for (pban = ban_list; pban != NULL; pban = pban->next)
    if (!str_suffix(pban->name, d->host)) return TRUE;
  return FALSE;
}
