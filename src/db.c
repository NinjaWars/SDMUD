...
void load_helps ( FILE * fp )
{
    ...
    for ( ;; )
    {
        ...
        if ( !str_cmp ( pHelp->keyword, "agreeting1" ) )
            help_greetinga = pHelp->text;
        if ( !str_cmp ( pHelp->keyword, "bgreeting2" ) )
            help_greetingb = pHelp->text;
        if ( !str_cmp ( pHelp->keyword, "cgreeting3" ) )
            help_greetingc = pHelp->text;
        if ( !str_cmp ( pHelp->keyword, "dgreeting4" ) )
            help_greetingd = pHelp->text;
        if ( !str_cmp ( pHelp->keyword, "egreeting5" ) )
            help_greetinge = pHelp->text;
        if ( !str_cmp ( pHelp->keyword, "fgreeting6" ) )
            help_greetingf = pHelp->text;
        if ( !str_cmp ( pHelp->keyword, "ggreeting7" ) )
            help_greetingg = pHelp->text;
        if ( !str_cmp ( pHelp->keyword, "hgreeting8" ) )
            help_greetingh = pHelp->text;
        if ( !str_cmp ( pHelp->keyword, "igreeting9" ) )
            help_greetingi = pHelp->text;
        if ( !str_cmp ( pHelp->keyword, "jgreeting10" ) )
            help_greetingj = pHelp->text;
        if ( !str_cmp ( pHelp->keyword, "kgreeting11" ) )
            help_greetingk = pHelp->text;
        if ( !str_cmp ( pHelp->keyword, "lgreeting12" ) )
            help_greetingl = pHelp->text;
        if ( !str_cmp ( pHelp->keyword, "mgreeting13" ) )
            help_greetingm = pHelp->text;
        if ( !str_cmp ( pHelp->keyword, "ngreeting14" ) )
            help_greetingn = pHelp->text;
        if ( !str_cmp ( pHelp->keyword, "ogreeting15" ) )
            help_greetingo = pHelp->text;
        if ( !str_cmp ( pHelp->keyword, "pgreeting16" ) )
            help_greetingp = pHelp->text;
        if ( !str_cmp ( pHelp->keyword, "qgreeting17" ) )
            help_greetingq = pHelp->text;
        if ( !str_cmp ( pHelp->keyword, "rgreeting18" ) )
            help_greetingr = pHelp->text;
        if ( !str_cmp ( pHelp->keyword, "sgreeting19" ) )
            help_greetings = pHelp->text;
        if ( !str_cmp ( pHelp->keyword, "tgreeting20" ) )
            help_greetingt = pHelp->text;
        if ( !str_cmp ( pHelp->keyword, "ugreeting21" ) )
            help_greetingu = pHelp->text;
        if ( !str_cmp ( pHelp->keyword, "vgreeting22" ) )
            help_greetingv = pHelp->text;
        if ( !str_cmp ( pHelp->keyword, "xgreeting23" ) )
            help_greetingx = pHelp->text;
        if ( !str_cmp ( pHelp->keyword, "ygreeting24" ) )
            help_greetingy = pHelp->text;
        if ( !str_cmp ( pHelp->keyword, "zgreeting25" ) )
            help_greetingz = pHelp->text;
...
void load_helps( FILE *fp, char *fname )
{
    ...
    for ( ; ; )
    {
        ...
        if ( !str_cmp ( pHelp->keyword, "zgreeting25" ) )
            help_greetingz = pHelp->text;
        if ( !str_cmp ( pHelp->keyword, "aagreeting1" ) )
            help_greetingaa = pHelp->text;
        if ( !str_cmp ( pHelp->keyword, "bbgreeting2" ) )
            help_greetingbb = pHelp->text;
        if ( !str_cmp ( pHelp->keyword, "ccgreeting3" ) )
            help_greetingcc = pHelp->text;
        if ( !str_cmp ( pHelp->keyword, "ddgreeting4" ) )
            help_greetingdd = pHelp->text;
        if ( !str_cmp ( pHelp->keyword, "eegreeting5" ) )
            help_greetingee = pHelp->text;
        if ( !str_cmp ( pHelp->keyword, "ffgreeting6" ) )
            help_greetingff = pHelp->text;
        if ( !str_cmp ( pHelp->keyword, "gggreeting7" ) )
            help_greetinggg = pHelp->text;
        if ( !str_cmp ( pHelp->keyword, "hhgreeting8" ) )
            help_greetinghh = pHelp->text;
        if ( !str_cmp ( pHelp->keyword, "iigreeting9" ) )
            help_greetingii = pHelp->text;
        if ( !str_cmp ( pHelp->keyword, "jjgreeting10" ) )
                ...
                plast++;
                {
                          ...
                          pHash = pHashPrev )
                    {
                        ...
                        if ( top_string[sizeof ( char * )] == pHash[0] &&
                             !str_cmp ( top_string + sizeof ( char * ) + 1,
                                        pHash + 1 ) )
                            return pHash;
                    }
   ...
   on pArea->next_sort. Sorted on get_area_level. */
{
    ...
    if(area_first==NULL)
    {
      bug("NO areas found/loaded.  NULL pointer for 'area_first' in sort_areas_by_level in db.c",0);
      return;
    }
...
//    output = new_buf (  );
    ...
    lo_level =
        convert_level ( arg1 ) ? URANGE ( 1, convert_level ( arg1 ),
                                          MAX_LEVEL ) : 0;
    hi_level =
...
//    add_buf(output,buf);
    buffer_strcat ( output, buf );
    sprintf ( buf, "{WWhite {xareas are the 'normal', levelling areas.{x\n\r" );
//    add_buf(output,buf);
    buffer_strcat ( output, buf );
    sprintf ( buf, "{gGreen {xareas are the (usually) inaccessible backbone areas.{x\n\r" );
//    add_buf(output,buf);
    buffer_strcat ( output, buf );
    sprintf ( buf, "Areas indicated '{RHARD{x' are for heroes and ancients. (These areas ARE hard!){x\n\r" );
//    add_buf(output,buf);
    buffer_strcat ( output, buf );
    sprintf ( buf, "The '{BAll Levels{x' areas are the big cities or roads of SD.{x\n\r" );
//    add_buf(output,buf);
    buffer_strcat ( output, buf );
    sprintf ( buf, "{c=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-={x\n\r" );
//    add_buf(output,buf);
...
CH_CMD ( do_mudinfo )
{
    ...
    buffer_strcat ( output, buf );
    sprintf ( buf, "          rooms total. Need assistance? Don't worry, SD has a big help\n\r" );
    buffer_strcat ( output, buf );
    sprintf ( buf, "          archive, containing {W%d{d helpfiles, easily accessable with the\n\r", top_help );
    buffer_strcat ( output, buf );
    sprintf ( buf, "          'help' command. If you feel like socializing, you can show lots\n\r" );
    buffer_strcat ( output, buf );
    sprintf ( buf, "          of emotions, {W%d{d to be exact. For a list of socials, type 'socials'.\n\r", maxSocial );
    buffer_strcat ( output, buf );
    sprintf ( buf, "\n\r          {dSince the last boot, a total of {W%d{d new players have created\n\r", creations_today );
    buffer_strcat ( output, buf );
    sprintf ( buf, "          a new character and {W%d{d characters have been rerolled.\n\r", rerolls_today );
    buffer_strcat ( output, buf );
    sprintf ( buf, "\n\r          Also since the last boot, a total of {W%ld{d connections have\n\r", connections_today );
    buffer_strcat ( output, buf );
...
CH_CMD ( do_dump )
{
    ...
    /* open file */
    fp = file_open ( "../data/dumps/mem.dmp", "w" );
    ...
    fp = file_open ( "../data/dumps/mob.dmp", "w" );
    ...
    fp = file_open ( "../data/dumps/obj.dmp", "w" );
    ...
    /* close file */
    new_fclose ( fp );
    send_to_char ( "Mem and obj files have been dumped to the data dir.\n\r", ch );
}
...
/* make sure we don't start an infinite loop */
    ...
    {
        if ( c1 == LOWER ( astr[ichar] ) &&
             !str_prefix ( bstr, astr + ichar ) )
        {
            found = TRUE;
            jchar = ichar;
            ichar = sstr1;
...
void randomize_entrances ( long code )
{
        ...
        for ( clannum = 0; clannum < MAX_CLAN; clannum++ )
        {
            ...
            for ( door = 0; door < 6; door++ )
            {
                if ( door == 5 )
                    ...
                    todoor = door - 2;
                portal =
                    get_obj_exit ( dir_name[door], pRoomIndex->contents );
                if ( ( portal != NULL ) &&
                     ( portal->item_type == ITEM_EXIT ) )
                {
            ...
            for ( ;; )
            {
                ...
                if ( ( pexit = pRoomIndex->exit[door] ) == NULL )
                {
                    ...
                    portal =
                        create_object ( get_obj_index ( OBJ_VNUM_EXIT ), 1 );
    ...
    if ( ( code == 0 ) || ( code == ROOM_VNUM_CHAIN ) )
    {
        ...
        sprintf ( buf, "The chain drifts off to the %s.\n\r",
                  dir_name[door] );
        for ( rch = pToRoomIndex->people; rch != NULL;
